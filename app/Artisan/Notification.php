  ...
  public static function send($user, $payLoad) {
    ...
    if (in_array($deviceType, self::DEVICES) && $token != "") {
      ...
      if ($deviceType === "android") {
        $apiKey = $config['mobile']['android']['api_key'];
        self::$headers = array('Authorization: key=' . $apiKey, 'Content-Type: application/json');
        return static::android($payLoad);

      } elseif ($deviceType === "ios") {
        static::$passPhrase = $config['mobile']['ios']['pass_phrase'];
